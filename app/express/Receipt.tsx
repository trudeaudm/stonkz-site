...
export function Receipt({
  ...
  const body = (
    <>
      ...
      {data.listed !== undefined && (
        <p>
          listed {data.listed.toString()} · side{' '}
          {data.sidePoolTokens?.toString() ?? '0'}
        </p>
      )}
      <p>
        side pool{' '}
  ...
  if (!chrome) return <div className="receipt-body">{body}</div>

  return (
    <div className="win receipt">
      <div className="tb">certificate.exe</div>
      <div className="body95">{body}</div>
    </div>
  )
}
